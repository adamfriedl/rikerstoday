...
import { Flex, PanelHeader, Text, Box, Panel, PanelFooter, Subhead} from 'rebass'
...
const Breakdown = props => {
  ...
  let to40 = props.data.filter(inmate => inmate.age > 21 && inmate.age <= 40).length
  ...
  let gangAffiliation = props.data.filter(inmate => inmate.srg_flg === 'Y').length
  ...
  return(
    <div>
      ...
      <Flex wrap>
        ...
        <TextStat
          title={'Mental Health'}
          stat={mentalHealthPercent}
        />

        <TextStat
          title={'Gang Affiliation'}
          stat={gangAffiliationPercent}
        />
